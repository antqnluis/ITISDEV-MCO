...
drop table if exists public.wellness_dimension_scores cascade;
...
create table public.students (
    ...
    constraint students_last_name_not_blank
        check (
            char_length(btrim(last_name)) between 1 and 100
        ),
...
create table public.student_profiles (
    ...
    -- Current DLSU academic term: 1, 2, or 3
    current_academic_term smallint not null,
    ...
    constraint student_profiles_academic_term_valid
        check (
            current_academic_term between 1 and 3
        ),

    constraint student_profiles_wellness_goals_count_valid
        check (
            cardinality(wellness_goals) <= 10
        ),
...
--
-- All five scores represent risk or concern:
-- 0   = low concern
-- 100 = high concern
--
-- These records are generated by the trusted backend using
-- rule-based calculations. Students can read, but not directly
-- create, update, or delete, their calculated scores.
-- ============================================================
...
create table public.wellness_dimension_scores (
    ...
    academic_engagement_score numeric(5,2) not null,
    personal_wellbeing_score numeric(5,2) not null,
    logistical_load_score numeric(5,2) not null,
    role_load_score numeric(5,2) not null,
    course_environment_score numeric(5,2) not null,
    ...
    constraint wellness_dimension_scores_check_in_student_fk
        foreign key (check_in_id, student_id)
        references public.weekly_check_ins(id, student_id)
        on delete cascade,

    constraint wellness_dimension_scores_check_in_unique
        unique (check_in_id),

    -- Supports a composite foreign key from ai_results so the
    -- AI result must reference the same student and check-in.
    constraint wellness_dimension_scores_reference_unique
        unique (id, student_id, check_in_id),

    constraint wellness_dimension_scores_academic_valid
        check (
            academic_engagement_score between 0 and 100
        ),

    constraint wellness_dimension_scores_wellbeing_valid
        check (
            personal_wellbeing_score between 0 and 100
        ),

    constraint wellness_dimension_scores_logistical_valid
        check (
            logistical_load_score between 0 and 100
        ),

    constraint wellness_dimension_scores_role_valid
        check (
            role_load_score between 0 and 100
        ),

    constraint wellness_dimension_scores_course_environment_valid
        check (
            course_environment_score between 0 and 100
        ),

    constraint wellness_dimension_scores_method_valid
        check (
    ...
    constraint wellness_dimension_scores_version_not_blank
        check (
            char_length(btrim(calculation_version)) > 0
        )
);
...
--
-- The five rule-based dimension scores are stored separately
-- in wellness_dimension_scores and referenced here as the
-- structured inputs used by the AI layer.
-- ============================================================
...
create table public.ai_results (
    ...
    check_in_id uuid not null,
    dimension_scores_id uuid not null,
    ...
    analysis_method text not null default 'rag_assisted',
    ...
    constraint ai_results_dimension_scores_fk
        foreign key (
            dimension_scores_id,
            student_id,
            check_in_id
        )
        references public.wellness_dimension_scores(
            id,
            student_id,
            check_in_id
        )
    ...
    constraint ai_results_dimension_scores_unique
        unique (dimension_scores_id),
...
create trigger wellness_dimension_scores_set_updated_at
before update on public.wellness_dimension_scores
for each row
execute function public.set_updated_at();
...
alter table public.wellness_dimension_scores
enable row level security;
...
--
-- Dimension scores can only be written by the trusted backend
-- using the Supabase service role.
-- ============================================================

create policy "Students can view their own dimension scores"
on public.wellness_dimension_scores
for select
to authenticated
using (
