...
function analysisUnavailable(req, res) {
...
router.post("/check-ins/analyze", analysisUnavailable);

/*
 * Original AI analysis pipeline preserved for future reactivation.
 *
 * It remains commented out because it requires the OpenAI SDK and a secured,
 * authenticated Supabase flow. The active route above fails safely without
 * loading unavailable dependencies or accessing student data.
 *
 * Original imports and initialization:
 *
 ...
 *         retrievedContext = matchedResources
 *           .map((resource) => `[Source: ${resource.title}] ${resource.content}`)
 *           .join("\n\n");
 *       }
 *     }
 *
 ...
 *         check_in_id,
 *         dimension_scores_id,
 *         swi_score: (scores.academic_engagement_score + scores.personal_wellbeing_score) / 2,
...
module.exports.analysisUnavailable = analysisUnavailable;
