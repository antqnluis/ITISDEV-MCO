  ...
  return (
    <AppShell>
      <DashboardPageHeader
        ...
        title="Settings"
        description="Keep your academic context, goals, and recurring responsibilities accurate so future insights reflect your real week."
      />

      {saved && <div role="status" className="mb-6 flex items-center gap-3 rounded-2xl border border-[#bfdbc7] bg-[#edf8f0] px-5 py-4 text-sm font-semibold text-[#2f6c47]"><span className="grid size-7 place-items-center rounded-full bg-[#4b8b62] text-white"><AppIcon name="check" className="size-4" /></span>Your prototype settings were saved.</div>}
      ...
      <form onSubmit={submit} className="grid gap-6 xl:grid-cols-[1fr_340px]">
        ...
        <aside className="space-y-6 xl:self-start">
          <section className="rounded-[20px] border border-[#dce5df] bg-[#edf5ef] p-6">
            ...
            <p className="mt-5 border-t border-[#cfdfd2] pt-4 text-xs leading-5 text-[#6f8178]">Only you can access your personal wellness data in this prototype.</p>
          </section>

          <section className="rounded-[20px] border border-[#e0e7e2] bg-white p-6">
            <h2 className="font-serif text-lg font-semibold text-[#173e30]">Prototype data</h2>
            <p className="mt-2 text-sm leading-6 text-[#718078]">Changes are kept in memory for this session and return to the seeded scenario after a refresh.</p>
          </section>
          ...
          <section className="rounded-[20px] border border-[#ecd4d0] bg-[#fffafa] p-6">
            ...
            <p className="mt-2 text-sm leading-6 text-[#8b6b67]">Signing out returns to the prototype login screen.</p>
