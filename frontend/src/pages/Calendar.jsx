...
function Calendar() {
  const { calendarEvents, saveEvent, deleteEvent } = usePrototypeData();
    ...
    <AppShell>
      ...
      <section className="mt-6 rounded-[20px] border border-[#e0e7e2] bg-white p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div><p className="text-xs font-bold uppercase tracking-[0.13em] text-[#789087]">Week overview</p><h2 className="mt-1 font-serif text-xl font-semibold text-[#173e30]">{visibleEvents.length} scheduled items</h2></div>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {visibleEvents.slice(0, 6).map((event) => (
            <article key={event.id} className="flex items-center gap-3 rounded-xl border border-[#e5ebe7] p-3.5">
              <span className={`size-2.5 shrink-0 rounded-full border ${eventStyle[event.event_type].split(" ")[1]}`} />
              <div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-[#29483b]">{event.title}</p><p className="mt-0.5 text-xs capitalize text-[#7b8984]">{event.event_type.replaceAll("_", " ")}</p></div>
              <button type="button" onClick={() => handleDelete(event)} aria-label={`Delete ${event.title}`} className="grid size-8 place-items-center rounded-lg text-[#9a7772] hover:bg-[#fff0ee] hover:text-[#a24c45]"><AppIcon name="trash" className="size-4" /></button>
            </article>
          ))}
        </div>
      </section>
